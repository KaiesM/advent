...
for game in games.replace(",", "").split("\n"):
    ...
    for hand in game.split(":")[1].strip().split(";"):
        if ("red" in hand and int(hand.split()[(hand.split().index("red") - 1)]) > 12) or ("green" in hand and int(hand.split()[(hand.split().index("green") - 1)]) > 13) or ("blue" in hand and int(hand.split()[(hand.split().index("blue") - 1)]) > 14):
            good_game = False
    if good_game:
