...
for row in calibration.split():
    ...
    for item in row:
        if item.isdigit():
            values.append(item)
    calibration_sum += int(str(values[0]) + str(values[-1]))
srow = "vgchkqhxrbjnlqnvpml77twonejcv"
values = []
if "one" in srow and "two" in srow:
    if srow.index("one") < srow.index("two"):
        srow = srow.replace("one", "1")
    else:
        srow = srow.replace("two", "2")
if "one" in srow:
    srow = srow.replace("one", "1")
if "two" in srow:
    srow = srow.replace("two", "2")
if "three" in srow:
    srow = srow.replace("three", "3")
if "four" in srow:
    srow = srow.replace("four", "4")
if "five" in srow:
    srow = srow.replace("five", "5")
if "six" in srow:
    srow = srow.replace("six", "6")
if "seven" in srow:
    srow = srow.replace("seven", "7")
if "eight" in srow:
    srow = srow.replace("eight", "8")
if "nine" in srow:
    srow = srow.replace("nine", "9")
for item in srow:
    if item.isdigit():
        values.append(item)
