...
for row in input.replace(".", " ").split("\n"):
    ...
    for index, item in enumerate(row):
        if item.isdigit():
            if number:
                number["item"]= number["item"] + item
                number["index"].append(str(index))
            else:
                number = {"item": item, "index": [str(index)]}
        else:
            if number:
                indexed_row.append(number)
            number = None
            if item == "*":
                indexed_row.append({"item": item, "index": str(index), "id": star_id, "adjacent": []})
                star_id += 1
    if number:
        indexed_row.append(number)
...
for row in indexed_rows:
    if previous_row:
        for p_item in previous_row:
            for item in row:
                if p_item["item"].isdigit() and not item["item"].isdigit():
                    if item["index"] in ([str(int(p_item["index"][0]) - 1)] + p_item["index"] + [str((int(p_item["index"][-1]) + 1))]):
                        item["adjacent"].append(p_item)
                        if item not in pot_gears:
                            pot_gears.append(item)
                elif item["item"].isdigit() and not p_item["item"].isdigit():
                    if p_item["index"] in  ([str(int(item["index"][0]) - 1)] + item["index"] + [(str(int(item["index"][-1]) + 1))]):
                        p_item["adjacent"].append(item)
                        if p_item not in pot_gears:
                            pot_gears.append(p_item)
    for item in row:
        for p_item in row:
            if item["item"].isdigit() and not p_item["item"].isdigit():
                if (int(item["index"][0]) -1 == int(p_item["index"])) or (int(item["index"][-1]) +1 == int(p_item["index"])):
                    p_item["adjacent"].append(item)
                    if p_item not in pot_gears:
                        pot_gears.append(p_item)
    previous_row = row
gears = [item for item in pot_gears if len(item["adjacent"]) == 2]

for e in gears:
    print(int(e["adjacent"][0]["item"]) * int(e["adjacent"][1]["item"]))
    sum += int(e["adjacent"][0]["item"]) * int(e["adjacent"][1]["item"])
print(sum)