games = """Game 1: 7 green, 4 blue, 3 red; 4 blue, 10 red, 1 green; 1 blue, 9 red
...
sum = 0
for game in games.replace(",", "").split("\n"):
    min_red=min_blue=min_green = 0
    for hand in game.split(":")[1].strip().split(";"):
        if "red" in hand:
            min_red = max(min_red, int(hand.split()[(hand.split().index("red") - 1)]))
        if "blue" in hand:
            min_blue = max(min_blue, int(hand.split()[(hand.split().index("blue") - 1)]))
        if "green" in hand:
            min_green = max(min_green, int(hand.split()[(hand.split().index("green") - 1)]))
    
    sum += min_red * min_blue * min_green
print(sum)

        
            # if min_red < (int(hand.split()[(hand.split().index("red") - 1)])):
                # min_red = int(hand.split()[(hand.split().index("red") - 1)])
            # if min_blue < (int(hand.split()[(hand.split().index("blue") - 1)])):
                # min_blue = int(hand.split()[(hand.split().index("blue") - 1)])
            # if min_green < (int(hand.split()[(hand.split().index("green") - 1)])):
                # min_green = int(hand.split()[(hand.split().index("green") - 1)])
